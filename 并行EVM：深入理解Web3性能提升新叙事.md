...
**乐观并行机制将交易的执行并行化**。Monad采用了乐观并行化实现并行执行。所不同的是它实现了自己的静态代码分析器，用于预测交易之间的依赖关系，仅在前置的依赖交易执行完毕后再调度后续交易执行，这样就大大减少了因验证失败导致的交易重复执行。随着项目的进展，核心团队还会继续探索更多的优化机制。

### 2.5 Neon

不同于L1公链设计各种各样的机制实现交易的并行执行，L2层解决方案往往采取“大树底下好乘凉”的方式，利用L1公链的并行执行能力然后做兼容和适配。
